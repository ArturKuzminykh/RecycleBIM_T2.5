...
model = ifcopenshell.open('D:/YandexDisk/04_Dev/UMinho_RecycleBIM/03_May 2024 Delivery/D2.6/TestIFC/17071996.ifc')
...
# Version to check with a console:
txt_file_path = 'checkQtoPossibility_output.txt'
with open(txt_file_path, 'w') as txt_file:
    for item in non_quantifiable_notes:
        txt_file.write('\n'.join(item) + '\n\n')

import csv
csv_file_path = 'checkQtoPossibility_output.csv'
with open(csv_file_path, 'w', newline='') as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(["Element GUID", "Element Class"])
    for guid in non_quantifiable_elements:
        csv_writer.writerow([guid, model.by_guid(guid).is_a()])